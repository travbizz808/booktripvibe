...
public class MainActivity extends android.app.Activity {
    private static final String HOME_URL = "https://booktripvibe.com/crm/login";
    ...
    private void clearWebCacheOncePerAppVersion() {
        ...
        SharedPreferences preferences = getSharedPreferences("booktripvibe_crm_app", MODE_PRIVATE);
    ...
    private void configureSystemBarsSafely() {
        ...
        int chromeColor = Color.rgb(9, 143, 247);
    ...
    private void configureWebView() {
        ...
        webView.addJavascriptInterface(new DownloadBridge(), "BookTripDownload");
        ...
        webView.setDownloadListener((url, userAgent, contentDisposition, mimeType, contentLength) -> {
            ...
            try {
                ...
                request.setDescription("BookTripVibe CRM");
    ...
    private void installGeneratedDownloadInterceptor(WebView view) {
        String script =
                "(function(){" +
                "if(window.__bookTripGeneratedDownloadFix)return;" +
                "window.__bookTripGeneratedDownloadFix=true;" +
                "function nameFor(a){return (a&&a.getAttribute&&a.getAttribute('download'))||('BookTripVibe-Itinerary-'+Date.now()+'.pdf');}" +
                "async function save(url,name){try{" +
                "var response=await fetch(url);var blob=await response.blob();var reader=new FileReader();" +
                "reader.onloadend=function(){BookTripDownload.saveBase64File(reader.result,name,blob.type||'application/pdf');};" +
                "reader.onerror=function(){BookTripDownload.reportError('Unable to read generated file');};reader.readAsDataURL(blob);" +
                "}catch(e){BookTripDownload.reportError(String(e));}}" +
    ...
    private void downloadGeneratedFile(String url, String fileName, String mimeType) {
        ...
        String script =
                ...
                "reader.onloadend=function(){BookTripDownload.saveBase64File(reader.result," +
                jsQuote(finalFileName) + ",blob.type||" + jsQuote(finalMimeType) + ");};" +
                "reader.onerror=function(){BookTripDownload.reportError('Unable to read generated PDF');};" +
    ...
    private String normalizeDownloadFileName(String fileName, String mimeType) {
        ...
        if (name.isEmpty() || name.equalsIgnoreCase("download") || name.startsWith("blob")) {
            name = "BookTripVibe-Itinerary-" + System.currentTimeMillis();
        }
